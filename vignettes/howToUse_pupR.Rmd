...
The demo data is from a survey carried out in the West Ice in 2012. When loading the demo data you get list containing two lists called `data` and `xycord`. The `data` list contains the pup counts for each photo for both the harp and the hooded population, along with lots of other information such as photo ID, which transect a given photo is etc. See table 1 for a complete overview of the variables in the data list. The `xycord` list contains positions in a cartesian format. 

```{r}
names(newname$data)

names(newname$xycord)
```
...
bdistHarp = birthDist(harpfname = "HarpStages2012.txt",
...
bdistHooded = birthDist(harpfname = "HarpStages2012.txt",
                  harpLengthStages = c(2.4,4.42,11.39),
                  harpKappa = 12.4,
                  hoodedfname = "HoodStages2012.txt",
                  hoodedLengthStages = c(2,1,4),
